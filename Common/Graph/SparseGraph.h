...
public:
  ...
  void RemoveEdges()
  {
    ...
    //non const class used to iterate through all the edges connected to a specific node. 
      ...
      private:                                                                
        ...
        const int                           NodeIndex;
        decltype(G.m_Edges.front().begin()) curEdge;
  ...
  //const class used to iterate through all the edges connected to a specific node. 
      ...
      private:                                                                
        ...
        const int                                 NodeIndex;
        decltype(G.m_Edges[NodeIndex].begin())    curEdge;
  ...
  friend class NodeIterator;
    ...
    private:
      ...
      //all the nodes that have a higher index. This method takes a node
      //iterator as a parameter and assigns the next valid element to it.
      const node_type * GetNextValidNode(typename NodeVector::const_iterator& it)
      {
        if (it == G.m_Nodes.end())
          return nullptr;

        if (it->Index() != invalid_node_index) 
          return &(*it);
...
bool SparseGraph<node_type, edge_type>::Save(std::ofstream& stream)const
{
  ...
  for (typename NodeVector::const_iterator curNode = m_Nodes.begin(); curNode!=m_Nodes.end(); ++curNode)
  {
    stream << *curNode;
  }
  ...
  {
    for (typename EdgeList::const_iterator curEdge = m_Edges[nodeIdx].begin();
