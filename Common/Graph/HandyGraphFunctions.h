...
{	
  ...
  //draw the nodes ConstNodeIterator
  typename graph_type::ConstNodeIterator NodeItr(graph);
  
  for (const auto * pN = NodeItr.begin(); pN; pN = NodeItr.next())
  {
    gdi->Circle(pN->Pos(), 2);
    ...
    for (const auto * pE=EdgeItr.begin();
        !EdgeItr.end();
        pE=EdgeItr.next())
    {
...
void WeightNavGraphNodeEdges(graph_type& graph, int node, double weight)
{
  ...
  for (const graph_type::EdgeType * pE = ConstEdgeItr.begin(); pE; pE = ConstEdgeItr.next())
  {
...
double CalculateAverageGraphEdgeLength(const graph_type& G)
{
  ...
  for (const auto * pN = NodeItr.begin(); !NodeItr.end(); pN=NodeItr.next())
  {
    typename graph_type::ConstEdgeIterator EdgeItr(G, pN->Index());
    for (const auto * pE = EdgeItr.begin(); !EdgeItr.end(); pE=EdgeItr.next())
    {
